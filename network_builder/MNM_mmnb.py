...
class MNM_demand_bus():
    ...
    def build_from_file(self, file_name):
        ...
        for i in range(len(log)):
            ...
            O_ID = int(words[0])
            D_ID = int(words[1])
            route_ID = int(words[2])
            demand = np.array(words[3:]).astype(float)
            total_l = len(demand)
    ...
    def build_from_file(self, file_name):
        ...
        for i in range(len(log)):
            ...
            if not flip:
                ...
                pickup_waiting_time = float(words[2])
...
class MNM_pathtable_bus_route():
    ...
    def build_from_file(self, path_table_file_name, route_file_time, w_ID=False, starting_ID=0):
        ...
        for i in range(len(log_path)):
            ...
            assert(int(words_path[0]) == int(words_route[0]))
            assert(int(words_path[1]) == int(words_route[1]))
            assert(int(words_path[2]) == int(words_route[2]))
            origin_node = int(words_path[0])
            destination_node = int(words_path[1])
            route_ID = int(words_path[2])
            ...
            tmp_node_list = list(map(lambda x : int(x), words_path[3:]))
            tmp_busstop_list = list(map(lambda x : int(x), words_route[3:]))
    ...
    def build_from_file(self, file_name, w_ID=False, starting_ID=0):
        ...
        for i in range(len(log)):
            ...
            if destination_node not in self.path_dict[origin_node].keys():
                ...
                self.add_pathset(tmp_path_set)
            # link IDs are integers
            tmp_link_list = list(map(lambda x : int(x), words[2:]))
...
class MNM_pathtable_pnr():
    ...
    def build_from_file(self, file_name, w_ID=False, starting_ID=0):
        ...
        for i in range(len(log)):
            ...
            origin_node = int(words[0])
            destination_node = int(words[1])
            mid_parkinglot_ID = int(words[2])
            mid_destination_node = int(words[3])
            if origin_node not in self.path_dict.keys():
                self.path_dict[origin_node] = dict()
            if destination_node not in self.path_dict[origin_node].keys():
                tmp_path_set = MNM_pathset_bustransit()
            ...
            tmp_node_list = list(map(lambda x : int(x), words[4:4+tmp_ind]))
            tmp_link_list = list(map(lambda x : int(x), words[4+tmp_ind:]))
...
class MNM_config():
    def __init__(self):
        ...
        self.type_dict = {
            ...
            'num_of_O': int, 
            'num_of_D': int, 
            'OD_pair_passenger': int,
            'OD_pair_driving': int, 
            'num_bus_routes': int,
            'num_of_bus_stop_physical': int, 
            'num_of_bus_stop_virtual': int,
            'num_of_bus_link': int,
            'num_of_walking_link': int,
            'num_of_parking_lot': int,
            'OD_pair_pnr': int,
            'OD_pair_bustransit': int,
            ...
            'num_bustransit_path': int,

            # MMDUE
            'driving': int,
            'transit': int,
            'pnr': int,
            'alpha1_driving': float,
            'alpha1_transit': float,
            'alpha1_pnr': float,
            'beta1': float,
            ...
            'carpool_cost_multiplier': float,
            'pnr_inconvenience': float,
            'bus_inconvenience': float,
            'parking_lot_to_destination_walking_time': float,
            'bus_fare': float,
            'metro_fare': float
        }
...
class MNM_network_builder():
    ...
    def read_link_bus_input(self, file_name):
        ...
        for i in range(len(log)):
            ...
            ID = int(words[0])
            from_busstop_ID = int(words[1])
            to_busstop_ID = int(words[2])
            length = float(words[3])
            fftt = float(words[4])
            route_ID = int(words[5])
            overlapped_dlink_ID_vec = list()
            for e in words[6:]:
                overlapped_dlink_ID_vec.append(int(e))
            overlapped_dlink_ID_vec = np.array(overlapped_dlink_ID_vec, dtype=int)
    ...
    def read_link_walking_input(self, file_name):
        ...
        for i in range(len(log)):
            ...
            ID = int(words[0])
            from_node_ID = int(words[1])
            to_node_ID = int(words[2])
            from_node_type = words[3]
            to_node_type = words[4]
            walking_type = words[5]
            walking_time = float(words[6])
    ...
    def read_busstop_virtual_input(self, file_name):
        ...
        for i in range(len(log)):
            ...
            virtual_busstop_ID = int(words[0])
            physical_busstop_ID = int(words[1])
            route_ID = int(words[2])
    ...
    def read_busstop_physical_input(self, file_name):
        ...
        for i in range(len(log)):
            ...
            physical_busstop_ID = int(words[0])
            link_ID = int(words[1])
            location = float(words[2])
            route_ID_vec = list()
            for e in words[3:]:
                route_ID_vec.append(int(e))
            route_ID_vec = np.array(route_ID_vec, dtype=int)
    ...
    def read_parkinglot_input(self, file_name):
        ...
        for i in range(len(log)):
            ...
            ID = int(words[0])
            dest_node_ID = int(words[1])
            price = float(words[2])
            price_surge_coeff = float(words[3])
            avg_parking_time = float(words[4])
            capacity = int(words[5])
    ...
    def load_from_folder(self, folder_path, config_file_name = 'config.conf', 
        ...
        self.od_mode_connectivity = np.zeros((len(self.demand_total_passenger.demand_list), 3), dtype=int)
        ...
        self.od_mode_connectivity = pd.DataFrame(data=self.od_mode_connectivity, columns=["driving", "bustransit", "pnr"], dtype=int)
    ...
    def get_mode_portion_matrix(self):
        ...
        for OD_idx, (O, D) in enumerate(self.demand_total_passenger.demand_list):
            demand_total = np.zeros(num_intervals, dtype=float)
