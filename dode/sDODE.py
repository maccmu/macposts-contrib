...
class SDODE():
  ...
  def estimate_demand_pytorch(self, init_scale=0.1, step_size=0.1,
    ...
    for i in range(max_epoch):
      ...
      print("Epoch:", i, "Loss:", loss / float(self.num_data))
      loss_list.append(loss / float(self.num_data))
      if save_folder is not None:
        pickle.dump([loss / float(self.num_data), q_e], open(os.path.join(save_folder, str(i)+'iteration.pickle'), 'wb'))
  ...
  def estimate_demand_cov(self, O_dist, D_dist, init_mean_scale=0.1,
    ...
    for i in range(max_epoch):
      ...
      loss_list.append(loss / float(self.num_data))
      if save_folder is not None:
        pickle.dump([loss / float(self.num_data), q_para], open(os.path.join(save_folder, str(i)+'iteration.pickle'), 'wb'))
